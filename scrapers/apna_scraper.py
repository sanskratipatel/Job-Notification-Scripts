"""
Apna.co scraper – popular India job platform, strong in tech roles.
Uses DuckDuckGo site search as Apna's API requires auth.
"""
...
def scrape(keyword: str = "python backend", **_) -> list[dict]:
    ...
    query = f'site:apna.co "{keyword}" developer OR engineer'
    try:
        data = {"q": query, "df": "w"}   # last week
        resp = requests.post(DDG_URL, data=data, headers=HEADERS, timeout=20)
        if resp.status_code != 200:
            logger.warning("Apna: DDG returned HTTP %d", resp.status_code)
            return jobs
    except Exception as e:
        logger.warning("Apna: request failed: %s", e)
        return jobs
    ...
    for result in results:
        try:
            ...
            company_match = re.search(r"at\s+([A-Z][A-Za-z0-9\s&]+?)(?:\s+is|\s+are|\s+–|\s+-|\.|,)", snippet)
            ...
            logger.debug("Apna job parse error: %s", e)

    logger.info("Apna: found %d jobs for '%s'", len(jobs), keyword)
    return jobs
